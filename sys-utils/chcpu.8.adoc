...
+
If CPU (de)configure is supported, disabled but configured CPUs should be avoided to prevent performance penalties. This is especially important when the system is in vertical polarization as disabling high performing(vertical high) CPUs without deconfiguring them would prevent hipervisor from reassign CPU polarizations, causing workloads to run on other(vertical low) CPUs.
...
+
*chcpu -g* is not supported on IBM z/VM, CPUs are always in configured state.
